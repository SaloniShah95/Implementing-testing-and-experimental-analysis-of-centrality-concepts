# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 22:25:51 2020
...
def runANDCompCode():
    if not os.path.exists('layerComposerv2'):
        f = open('layerComposerv2.cpp', 'w')
        f.write(prog)
        f.close()
        subprocess.call(["g++", "layerComposerv2.cpp", "-olayerComposerv2", "-std=c99", '-w', '-Ofast'])
    
    #subprocess.check_output(["./foo"], stdin = sys.stdin)
    y=subprocess.run(["./layerComposerv2"], stdin = sys.stdin, stdout=PIPE, stderr=PIPE)
    #print(y)
